...
# Структуры
Структура - тип данных, объединяющий переменные различных типов данных в одну группу.  
...
```
В данном примере структура, описывающая автора.  
У нее есть поля:  
- Название - строка;
- Автор - строка;
- Цена - целое число;  
Создание объекта:
```C++
Book book1 = { "abcd", "dbca", 300 };
```
Для обращения к полям структуры используем ```.```:  
```C++
printf("Название: %s\nАвтор: %s\nЦена: %d\n",
    book1.name, book1.author, book1.price);
```

Полями структуры могут быть и составные типы данных:  
```C++
struct Student {
    char name[50];
    int grades[5]; // Массив оценок
};
```
Также мы можем не инициализировать поля структуры при создании объекта, а сделать это позже:  
```C++
Student student;

snprintf(student.name, sizeof(student.name), "Иван");
student.grades[0] = 4;
student.grades[1] = 5;
student.grades[2] = 3;
student.grades[3] = 5;
```
Так как мы не можем просто присвоить значение строке (в отличие от числа, так работает с оценками), необходимо использовать функцию ```snprintf```, которая безопасно заполнит строку данными.

## Функции в структурах
Также в структуре можно определять функции:
```C++
struct Student {
    char name[50];
    int grades[5]; // Массив оценок
...
```
Например, можно сделать функцию, выводящую всю информацию объекта.  
**Важно:** функции внутри структуры имеют доступ только к тому, что определено в этой структуре.  
То есть её поля(```name```, ```grades```) и методы(```printStudent```).

## Хранение объектов
Хранить созданные объекты можно в массиве, динамическом массиве и других структурах данных.  
Например, можем создать двух студентов и сохранить их в массив:  
```C++
Student st1 = { "Иван",  {3,5,5,4,3} };
Student st2 = { "Петр",  {5,5,5,4,5} };

Student students[10] = { st1, st2 };
```
Или сделать так (инициализировать переменные при создании структуры):
```C++
Student students[10] = {
{ "Иван",  {3,5,5,4,3} },
{ "Петр",  {5,5,5,4,5} }
