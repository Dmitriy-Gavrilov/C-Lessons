...
Для работы с такими фалами к режиму открытия файла добавляется ```b```.
Пример открытия бинарного файла в режиме чтения:  
```C++
fopen_s(&file, "test.txt", "rb")
```

Для обработки данных в таком файле обычно использую функции ```fread``` и ```fwrite``` описанные выше.
